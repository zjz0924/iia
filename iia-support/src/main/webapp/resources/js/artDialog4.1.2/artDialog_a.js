 ...
 * @param {String} 是否显示icon图标包括：['alert', 'error', 'succeed', 'loading', 'confirm']
...
artDialog.tips = function (content, time, icon, callback) {
	...
	var _conf = {
		...
		padding:0,
		init: function(){
			this.DOM.iconBg.css({width: '32px', height: '32px'});
		},
		close: callback
	};
	if(icon && typeof(icon) === 'string'){
		_conf['icon'] = icon == 'loading' ? 'b_loading' : '32X32/' + icon;
		//_conf['lock'] = icon == 'loading' ? true : false;
	}
	return _proxyDialog(_conf).content('<div style="padding: 0 1em;">' + content + '</div>').time(typeof(time) === 'number' ? time : 2 );
};
...
*/
(function (config) {
    config['lock'] = true;// 是否锁屏
    config['background'] = '#000000';// 遮罩颜色;#000000，#600
    config['opacity'] = 0.7;// 遮罩透明度
    config['duration'] = 400;// 遮罩透明度渐变动画速度
    config['fixed'] = true;// 是否静止定位
    config['focus'] = false;
})(art.dialog.defaults); 
